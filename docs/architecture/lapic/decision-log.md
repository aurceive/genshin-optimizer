...
## D-007 UI Solve Configuration Policy

- Status: accepted
- Scope: runtime configuration for user-facing optimisation solves

### D-007 Decision

UI-driven solves use a minimal-output configuration:

- `skipIntermediateCertificates: true` — intermediate certificates are not emitted; FinalOptimalityCertificate is always emitted.
- No diagnostic subscription — the runtime's diagnostic broadcast operates identically with zero listeners. Solve failures surface through `outcome.state === 'failed'` and `outcome.error`.

### D-007 Rationale

- The primary output of a UI solve is the top-N candidate set plus a completion proof (FinalOptimalityCertificate).
- Intermediate certificates (BoundPruneCert, DominanceCert) are per-decision audit artifacts for algorithm development. Emitting thousands of them in a browser Worker exhausts memory and provides no user-facing value.
- Diagnostic trace events (EmitCertificate, PublishArtifacts, Progress) are high-frequency telemetry. Streaming them across the Worker↔main-thread postMessage boundary crashes the page. The UI shows a solve-completion summary, not a real-time event stream.
- Benchmarks, regression tests, and certification tooling run with full certificate and diagnostic emission. The runtime configuration space already supports this; no special "UI mode" flag is needed.

### D-007 Constraint

This decision applies to browser-hosted UI solves. CLI tooling, test harnesses, and certification pipelines should use the full-emission default.
