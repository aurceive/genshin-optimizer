...
describe('LapicPriorityQueue', () => {
  it('dequeues highest priority first (higher upperBound)', () => {
    ...
    q.enqueue({ workUnit: workUnit('low', priority('100')), enqueuedAtSequence: 1 })
    q.enqueue({ workUnit: workUnit('high', priority('900')), enqueuedAtSequence: 2 })
    q.enqueue({ workUnit: workUnit('mid', priority('500')), enqueuedAtSequence: 3 })
  ...
  it('peek does not remove the item', () => {
    const q = createLapicPriorityQueue()
    q.enqueue({ workUnit: workUnit('a', priority('999')), enqueuedAtSequence: 1 })
  ...
  it('remove extracts a specific item', () => {
    const q = createLapicPriorityQueue()
    q.enqueue({ workUnit: workUnit('a', priority('900')), enqueuedAtSequence: 1 })
    q.enqueue({ workUnit: workUnit('b', priority('800')), enqueuedAtSequence: 2 })
    q.enqueue({ workUnit: workUnit('c', priority('700')), enqueuedAtSequence: 3 })
  ...
  it('toSortedArray returns items in priority order', () => {
    const q = createLapicPriorityQueue()
    q.enqueue({ workUnit: workUnit('c', priority('100')), enqueuedAtSequence: 3 })
    q.enqueue({ workUnit: workUnit('a', priority('900')), enqueuedAtSequence: 1 })
    q.enqueue({ workUnit: workUnit('b', priority('500')), enqueuedAtSequence: 2 })
  ...
  it('handles many items correctly', () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      expect(entry.workUnit.priority.upperBoundOrderingDigest <= prev).toBe(true)
