...
export function validateLapicSolveOrchestrationConfig(
  ...
  if (!isRecord(config))
    return createRuntimeFailure(
      'Orchestration config must be a record.',
      ['orchestrationConfig']
    )
  ...
  if (typeof (config.artifactStore as Record<string, unknown>).write !== 'function')
    return createRuntimeFailure(
      'Artifact store must have a write method.',
      ['artifactStore', 'write']
    )

  if (typeof (config.artifactStore as Record<string, unknown>).read !== 'function')
    return createRuntimeFailure(
      'Artifact store must have a read method.',
      ['artifactStore', 'read']
    )
...
): LapicValidationResult<{ from: LapicOrchestrationState; to: LapicOrchestrationState }> {
...
export function validateLapicOrchestrationOutcome(
  ...
  if (!isRecord(outcome))
    return createRuntimeFailure(
      'Orchestration outcome must be a record.',
  ...
  if (outcome.state === 'completed') {
    ...
    if (outcome.error)
      return createRuntimeFailure(
        'Completed outcome must not include error.',
  ...
  if (outcome.state === 'paused') {
    if (!outcome.solveOutcome)
      return createRuntimeFailure(
        'Paused outcome must include solveOutcome.',
        ['orchestrationOutcome', 'solveOutcome']
      )
    if (outcome.error)
      return createRuntimeFailure(
        'Paused outcome must not include error.',
        ['orchestrationOutcome', 'error']
      )
  }
  ...
  if (outcome.state === 'failed') {
    if (!outcome.error)
      return createRuntimeFailure(
        'Failed outcome must include error.',
        ['orchestrationOutcome', 'error']
      )
  }
  ...
  if (outcome.state === 'cancelled') {
    if (outcome.error)
      return createRuntimeFailure(
        'Cancelled outcome must not include error.',
        ['orchestrationOutcome', 'error']
      )
  }
